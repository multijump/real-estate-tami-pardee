<template>
  <section class="PageVideoRow">
    <h2 class="PageVideoRow__title">
      {{ content.section_headline }}
    </h2>

    <div class="PageVideoRow__wrapper">
      <div
        v-for="(item, index) in content.video_entries"
        :key="index"
        class="PageVideoRow__item"
      >
        <img
          class="PageVideoRow__image"
        ...
        <PlayImage
          v-if="item.video_url"
          class="PageVideoRow__play"
          @click.prevent="openVideoModal(item.video_url)"
        />

        <h3 class="PageVideoRow__name">
          {{ item.headline }}
        </h3>

        <p class="PageVideoRow__subname">
          {{ item.subheadline }}
        </p>
      </div>
    </div>
