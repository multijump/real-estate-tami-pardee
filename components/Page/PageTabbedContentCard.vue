<template>
  <div class="PageTabbedContentCard">
    <AppTabsList
      id="tabNewsItems"
      ref="newsList"
      :list="tabs"
    ...
    <div class="PageTabbedContentCard__fixed">
      <ArrowNextImage
        class="PageTabbedContentCard__arrow"
        @click.prevent="nextTab"
      />
    </div>

    <div class="PageTabbedContentCard__content">
      <div
        v-for="(item, index) in tabs"
        v-show="activeItem === index + 1"
        :key="index"
        class="PageTabbedContentCard__wrapper"
        :class="{ 'PageTabbedContentCard__wrapper--active': activeItem === index + 1 }"
      >
        <p
          v-if="item.tab_title"
          class="PageTabbedContentCard__title"
          v-html="item.tab_title"
        />

        <p
          v-if="item.tab_body"
          class="PageTabbedContentCard__description"
          v-html="item.tab_body.replace(/\n/g, '<br>')"
        />

        <div class="PageTabbedContentCard__action">
          <AppLinkButton
            v-if="item.link_primary"
            color="pink"
